...
template <typename T>
void DoublyLinkedList<T>::index_insert(const T &info, int index) {
    // 1. Caso: Índice maior que o tamanho da lista -> Insere no INÍCIO
    // (Assumimos que o usuário passa um valor que excede o limite esperado)
    // Nota: Como não temos um contador de tamanho, tratamos isso como prioridade.
    if (index >= get_size()) { // Supondo que você tenha um método get_size()
        Node<T> * new_node = new Node<T>(info);
        new_node->next = head;
        new_node->previous = nullptr;

        if (head != nullptr) head->previous = new_node;
        else tail = new_node;
    ...
    // 2. Caso: Índice 0 -> Insere no FINAL
    if (index == 0) {
        Node<T> * new_node = new Node<T>(info);
    ...
    // 3. Caso: Inserção no meio (índice intermediário)
    Node<T> * current = head;
    int count = 0;
